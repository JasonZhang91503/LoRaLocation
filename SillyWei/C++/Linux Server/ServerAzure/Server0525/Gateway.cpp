...
void stateUpdate(int id, int state) {
	...
	stmt->execute("UPDATE transport SET firstNotify=true WHERE _id=" + idString);
}
...
void getNewTransport(vector<string> &message) {
    ...
    header = "\2  \2";
    res = stmt->executeQuery("SELECT * FROM transport WHERE requireTime = \"" + time + "\"AND state=0");
	for (int i = 0; res->next(); i++) {
		...
		message[i].append(res->getString("packetKey") + ",");
