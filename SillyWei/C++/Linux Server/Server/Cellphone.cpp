...
bool createTransport(string strTemp) {
    ...
    cout << "INSERT INTO transport VALUE(default,'" + strRequireTime + "','" +
        ...
        strStartLocation + "," + strDestLocation + ",1,0,'" + makeKey(4) + "','" + strRemark +"')" << endl;
    // create a transport
    if (stmt->executeUpdate("INSERT INTO transport VALUE(default,'" + strRequireTime + "','" +
        strRequireTime + "'+interval 15 MINUTE," + strSender +"," + strReceiver + "," +
        strStartLocation + "," + strDestLocation + ",1,0,'" + makeKey(4) + "','" + strRemark +"')")) {
        cout << "here" << endl;
        return true;
    }
    return false;
...
void askTransportSend(string strTemp, string &strSendBuff, int intId) {
    ...
    while (res->next()) {
        ...
        strTemp.append(res->getString("key") + "$");
...
void handleCellphoneBack(int intSocketClient) {
    ...
    while (true) {
        ...
        strSendBuff = "2, \n";
        if (getArriveTransport(strSendBuff, intId)) {
            send(intSocketClient, &strSendBuff[0], strSendBuff.size(), 0);
            cout << endl << "CellphoneBack socket: " << intSocketClient << ", have a transport" << endl << "wait for the next search" << endl;
        }else {
...
bool getArriveTransport(string &strSendBuff, int intId) {
    ...
    res = stmt->executeQuery("SELECT * FROM transport WHERE state=1 AND requireTime='" +
        strTime +"'AND sender=" + to_string(intId));
    if (res->next()) {
        strSendBuff[2] = '1';
        delete res;
        return true;
    ...
    res = stmt->executeQuery("SELECT * FROM transport WHERE state=3 AND requireTime='" +
        strTime + "'AND receiver=" + to_string(intId));
    if (res->next()) {
        strSendBuff[2] = '0';
        delete res;
        return true;
